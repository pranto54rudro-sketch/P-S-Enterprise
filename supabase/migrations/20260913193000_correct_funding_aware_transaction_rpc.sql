...
begin
 ...
 if p_type='Buying' then
   v_units:=round(p_amount/v_unit_cost,4);
   if v_units<=0 then raise exception 'Buying amount is too small for one unit'; end if;
   v_people:=least(greatest(coalesce(p_people_funded,0),0),p_amount);
   v_owner:=p_amount-v_people;
   select greatest(0,coalesce((select opening_capital from public.business_settings limit 1),0)-coalesce(sum(owner_funded),0)) into v_owner_available from public.transactions where business_code='secondary' and type='Buying';
   ...
   for v_lot in select bl.* from public.buying_lots bl where bl.business_code='secondary' and bl.units>0 order by bl.lot_date,bl.created_at,bl.id for update loop
     ...
     v_take:=least(v_remaining,v_lot.units);
     ...
     v_people_cost:=v_people_cost+v_take*(v_lot.people_units/nullif(v_lot.units,0))*v_lot.unit_cost;
     v_owner_cost:=v_owner_cost+v_take*(v_lot.owner_units/nullif(v_lot.units,0))*v_lot.unit_cost;
